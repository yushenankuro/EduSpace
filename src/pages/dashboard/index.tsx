...
const Dashboard: React.FC = () => {
  ...
  const routerRef = useRef(router); // ✅ Fix scroll: simpan router di ref supaya tidak trigger re-render
  ...
  const [formData, setFormData] = useState<FormData>({
    name: '', email: '', nisn: '', birth_date: '', student_gender: ''
  });
  const [teacherFormData, setTeacherFormData] = useState<TeacherFormData>({
    name: '', email: '', subjects: [], teacher_gender: ''
  });
  ...
  // ✅ Fix scroll: gunakan [] bukan [router] — router disimpan di ref
  useEffect(() => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (error || !roleData) {
          setUserRole('guru');
        } else if (roleData.role !== 'admin' && roleData.role !== 'guru') {
          alert('Akses ditolak!');
        ...
        // ✅ Selalu jalan, loader pasti hilang
    ...
    checkAuth();
  }, []); // ✅ [] kosong — tidak akan re-run saat scroll
  ...
  const openStudentModal = (student?: Student) => {
    if (student) {
      ...
      setFormData({ name: student.name, email: student.email, nisn: student.nisn, birth_date: student.birth_date, student_gender: student.student_gender });
  ...
  const openTeacherModal = (teacher?: Teacher) => {
    if (teacher) {
      ...
      setTeacherFormData({ name: teacher.name, email: teacher.email, subjects: teacher.subjects || [], teacher_gender: teacher.teacher_gender });
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!file.type.startsWith('image/')) { setError('File harus berupa gambar!'); setTimeout(() => setError(''), 3000); return; }
    if (file.size > 2 * 1024 * 1024) { setError('Ukuran file maksimal 2MB!'); setTimeout(() => setError(''), 3000); return; }
    setPhotoFile(file);
  ...
  const uploadPhoto = async (file: File, identifier: string, bucket: string): Promise<string> => {
    try {
      ...
      const { error } = await supabase.storage.from(bucket).upload(fileName, file, { cacheControl: '3600', upsert: true, contentType: file.type });
      if (error) throw error;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.email || !formData.nisn || !formData.birth_date || !formData.student_gender) {
      setError('Semua field harus diisi!'); setTimeout(() => setError(''), 3000); return;
    }
    if (formData.nisn.length !== 10) {
      setError('NISN harus 10 digit!'); setTimeout(() => setError(''), 3000); return;
    }
    try {
      setUploading(true);
      let photoUrl = '';
      if (photoFile) { try { photoUrl = await uploadPhoto(photoFile, formData.nisn, 'student-photos'); } catch {} }
      const studentData = { ...formData, class: 'XI RPL 1', photo_url: photoUrl || null };
      let dbError;
      if (editingId) {
  ...
  const handleSubmitTeacher = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!teacherFormData.name || !teacherFormData.email || teacherFormData.subjects.length === 0) {
      setError('Semua field harus diisi dan minimal pilih 1 mata pelajaran!'); setTimeout(() => setError(''), 3000); return;
    }
    try {
      setUploadingTeacher(true);
      let photoUrl = '';
      if (teacherPhotoFile) { try { photoUrl = await uploadPhoto(teacherPhotoFile, teacherFormData.email, 'teacher-photos'); } catch {} }
      const teacherData = { ...teacherFormData, photo_url: photoUrl || null };
      if (editingTeacherId) {
        const { error } = await supabase.from('teachers').update(teacherData).eq('id', editingTeacherId);
        if (error) throw error;
  ...
  const filteredTeachers = teachers.filter(t =>
    ...
    (t.subjects && t.subjects.some(s => s.toLowerCase().includes(searchTeacher.toLowerCase())))
  );
  ...
  const currentStudents = filteredStudents.slice(startIndexStudents, startIndexStudents + itemsPerPage);
  ...
  const currentTeachers = filteredTeachers.slice(startIndexTeachers, startIndexTeachers + itemsPerPage);
              ...
              <div className="text-center md:text-left">
                ...
                <p className="text-teal-100">
                  ...
                  <span className={`ml-2 px-3 py-1 rounded-full text-sm font-medium ${
                    userRole === 'admin' ? 'bg-white/20 text-white' : 'bg-white/20 text-white'
                  }`}>
                    {userRole === 'admin' ? 'Admin' : 'Guru'}
                  </span>
                </p>
              </div>
                  ...
                  {loading ? (
                    ...
                    [...Array(5)].map((_, i) => (
                      <tr key={i} className="animate-pulse">
                        <td className="px-6 py-4"><div className="h-4 bg-slate-200 rounded w-6"></div></td>
                        <td className="px-6 py-4"><div className="w-12 h-12 bg-slate-200 rounded-full"></div></td>
                        ...
                        <td className="px-6 py-4"><div className="flex gap-2 justify-center"><div className="h-8 bg-slate-200 rounded w-16"></div><div className="h-8 bg-slate-200 rounded w-16"></div></div></td>
                      </tr>
                    ))
                    ...
                    currentStudents.map((student, index) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span className="font-mono text-slate-700 bg-slate-100 px-3 py-1 rounded-lg">{student.nisn}</span>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex justify-center gap-2">
                            <button onClick={() => openStudentModal(student)} className="bg-amber-500 text-white px-4 py-2 rounded-lg hover:bg-amber-600 transition-all duration-300 flex items-center gap-2">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                              Edit
                            </button>
                            <button onClick={() => handleDelete(student.id, 'student')} className="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition-all duration-300 flex items-center gap-2">
              ...
              <div className="bg-slate-50 px-6 py-6 border-t border-slate-200">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                  <p className="text-sm text-slate-600">
                    Menampilkan <span className="font-semibold text-teal-600">{startIndexStudents + 1}-{Math.min(startIndexStudents + itemsPerPage, filteredStudents.length)}</span> dari{' '}
                    <span className="font-semibold text-teal-600">{filteredStudents.length}</span> siswa
                  </p>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setCurrentPageStudents(p => p - 1)} disabled={currentPageStudents === 1} className="px-4 py-2 rounded-lg bg-slate-200 text-slate-700 hover:bg-slate-300 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300">← Prev</button>
                    <span className="px-4 py-2 bg-gradient-to-r from-teal-500 to-cyan-500 text-white rounded-lg">{currentPageStudents}</span>
                    <button onClick={() => setCurrentPageStudents(p => p + 1)} disabled={currentPageStudents === totalPagesStudents} className="px-4 py-2 rounded-lg bg-slate-200 text-slate-700 hover:bg-slate-300 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300">Next →</button>
                  </div>
                </div>
              </div>
            )}
          ...
          >
            ...
            <div className="px-6 py-6 border-b border-slate-200 bg-gradient-to-r from-purple-50 to-pink-50">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex gap-3 w-full md:w-auto">
                  ...
                  >
                    <span>+</span><span>Tambah Guru</span>
                  </button>
                </div>
              </div>
            </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-2">
                            {teacher.subjects && teacher.subjects.map((subject, idx) => (
                              <span key={idx} className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full text-sm font-medium">{subject}</span>
                            ))}
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex justify-center gap-2">
                            <button onClick={() => openTeacherModal(teacher)} className="bg-amber-500 text-white px-4 py-2 rounded-lg hover:bg-amber-600 transition-all duration-300 flex items-center gap-2">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                              Edit
                            </button>
                            <button onClick={() => handleDelete(teacher.id, 'teacher')} className="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition-all duration-300 flex items-center gap-2">
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                              Hapus
                            </button>
                          </div>
            ...
            {filteredTeachers.length > 0 && (
              <div className="bg-slate-50 px-6 py-6 border-t border-slate-200">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                  <p className="text-sm text-slate-600">
                    Menampilkan <span className="font-semibold text-purple-600">{startIndexTeachers + 1}-{Math.min(startIndexTeachers + itemsPerPage, filteredTeachers.length)}</span> dari{' '}
                    <span className="font-semibold text-purple-600">{filteredTeachers.length}</span> guru
                  </p>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setCurrentPageTeachers(p => p - 1)} disabled={currentPageTeachers === 1} className="px-4 py-2 rounded-lg bg-slate-200 text-slate-700 hover:bg-slate-300 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300">← Prev</button>
                    <span className="px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-lg">{currentPageTeachers}</span>
                    <button onClick={() => setCurrentPageTeachers(p => p + 1)} disabled={currentPageTeachers === totalPagesTeachers} className="px-4 py-2 rounded-lg bg-slate-200 text-slate-700 hover:bg-slate-300 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300">Next →</button>
                  </div>
                </div>
              </div>
            )}
            ...
            >
              <div className="bg-gradient-to-r from-teal-500 to-cyan-500 text-white p-6 rounded-t-2xl">
                <div className="flex justify-between items-center">
                  <h3 className="text-xl font-bold">{editingId ? 'Edit Data Siswa' : 'Tambah Siswa Baru'}</h3>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-200">
                  ...
                  <button type="button" onClick={closeStudentModal} className="px-8 py-3.5 bg-slate-200 text-slate-700 rounded-xl hover:bg-slate-300 transition-all duration-300 font-medium">Batal</button>
                </div>
              </form>
            </motion.div>
          </motion.div>
          ...
          >
            ...
            >
              <div className="bg-gradient-to-r from-purple-500 to-pink-500 text-white p-6 rounded-t-2xl">
                <div className="flex justify-between items-center">
                  <h3 className="text-xl font-bold">{editingTeacherId ? 'Edit Data Guru' : 'Tambah Guru Baru'}</h3>
              ...
              <form onSubmit={handleSubmitTeacher} className="p-6 space-y-6">
                <div className="flex flex-col sm:flex-row gap-6 items-center sm:items-start">
                  <div className="w-40 h-40 rounded-xl overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 border-4 border-white shadow-lg">
                    {teacherPhotoPreview ? <img src={teacherPhotoPreview} alt="Preview" className="w-full h-full object-cover" /> : (
                      <div className="w-full h-full flex items-center justify-center text-slate-400">
                        <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                      </div>
                    )}
                ...
                <div>
                  <label className="block text-slate-700 font-medium mb-2">Mata Pelajaran <span className="text-red-500">*</span></label>
                  <Select
                    isMulti
                    ...
                    className="text-left"
                    styles={{
                      control: (base) => ({ ...base, padding: '6px', borderRadius: '12px', borderColor: '#cbd5e1', '&:hover': { borderColor: '#a855f7' } }),
                      multiValue: (base) => ({ ...base, backgroundColor: '#f3e8ff' }),
                      multiValueLabel: (base) => ({ ...base, color: '#7e22ce', fontWeight: '500' })
                    }}
                  />
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-200">
                  ...
                  <button type="button" onClick={closeTeacherModal} className="px-8 py-3.5 bg-slate-200 text-slate-700 rounded-xl hover:bg-slate-300 transition-all duration-300 font-medium">Batal</button>
                </div>
              </form>
            </motion.div>
          </motion.div>
